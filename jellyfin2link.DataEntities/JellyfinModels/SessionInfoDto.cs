...
namespace jellyfin2link.DataEntities.JellyfinModels
{
    ...
    /// Session info DTO.
    /// </summary>
    [DebuggerDisplay("{PlayState} User: {UserName} IsActive: {IsActive} {NowPlayingItem?.Name}")]
    ...
    //public class DashlessGuidConverter : JsonConverter<Guid>
    //{
    ...
    //            return guid;
    //        if (Guid.TryParseExact(str, "N", out guid)) // handles dashless format
    //            return guid;
    ...
    //    {
    //        writer.WriteStringValue(value.ToString("N")); // write without dashes
    //    }
    //}
    ...
    public class DashlessGuidConverter : JsonConverter<Guid>
    {
        public override Guid Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            if (Guid.TryParseExact(str, "N", out guid)) // dashless
                return guid;
